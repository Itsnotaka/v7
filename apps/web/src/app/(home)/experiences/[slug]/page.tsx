...
export default async function Page({ params }: Props) {
  ...
  const sections: NavItem[] = item.works.map((work) => ({ id: slugify(work.title), label: work.title }));
    ...
    <PageSection id="top">
      <div className="col-span-8 animate-article-enter">
        <div className="relative flex h-[64vh] w-full items-end overflow-hidden shadow-xs ring ring-border">
          <ExperienceMedia item={item} sizes="(min-width: 1024px) 64rem, 100vw" className="h-full w-full object-contain" priority />
        </div>
      </div>

      <header className="col-span-8 tablet:col-span-6 tablet:col-start-2 flex flex-col gap-3 pt-10 pb-8 animate-article-enter" style={{ animationDelay: "80ms" }}>
      ...
      <div className="col-span-8 tablet:col-span-6 tablet:col-start-2 flex flex-col gap-6 animate-article-enter" style={{ animationDelay: "160ms" }}>
        {item.works.map((work) => (
          <div key={work.title} id={slugify(work.title)} className="flex flex-col tablet:flex-row gap-6 bg-muted p-5 shadow-xs ring ring-border">
      ...
      <div className="col-span-8 tablet:col-span-6 tablet:col-start-2 flex items-center justify-between border-t border-border pt-6 mt-4 animate-article-enter" style={{ animationDelay: "240ms" }}>
