...
export const NYTE_TOOLTIP_DEFAULT_VARIANTS = {
  side: "top",
} as const;

export type NyteTooltipSide = keyof typeof NYTE_TOOLTIP_VARIANTS.side;

export interface NyteTooltipVariantsProps {
  side?: NyteTooltipSide;
}

export function tooltipVariants({
  side = NYTE_TOOLTIP_DEFAULT_VARIANTS.side,
}: NyteTooltipVariantsProps = {}) {
  return cn(
    "flex origin-[var(--transform-origin)] flex-col rounded-md bg-card px-2.5 py-1.5 text-sm text-card-foreground shadow-lg outline outline-1 outline-border",
    "transition-[transform,opacity] duration-150 ease-out",
    "data-[starting-style]:scale-90 data-[starting-style]:opacity-0",
    "data-[ending-style]:scale-90 data-[ending-style]:opacity-0",
    "data-[instant]:duration-0",
    NYTE_TOOLTIP_VARIANTS.side[side].classes,
  );
}
    ...
    <TooltipBase.Root {...props}>
      ...
      <TooltipBase.Portal>
        <TooltipBase.Positioner align={align} side={side} sideOffset={10}>
          <TooltipBase.Popup className={tooltipVariants({ side })}>
            <TooltipBase.Arrow
              className={cn(
                "flex",
